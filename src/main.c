...
main ()
{
  ...
  SDL_Window *window
      = SDL_CreateWindow ("Custom Renderer", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, WIDTH, HEIGHT, 0);
  if (!window)
    {
  ...
  size_t MAXIMUM_SCREEN_MEMORY_REQUIREMENT
      = pixels_buffer_size + screen_vectors_size + world_vectors_size + triangle_colours_size;
  ...
  const int floor_triangles = 20;
  int triangle_number = cube_triangles + floor_triangles;

  float cube_size = 14.0f;
  vector_3d bottom_left_corner = { -7.0f, -7.0f, -30.0f };

  // 0 = cube rests on the floor.
  // Positive values = gap between cube bottom and floor.
  float cube_height_above_floor = 2.0f;

  vector_3d floor_centre = { bottom_left_corner.x + cube_size / 2.0f, bottom_left_corner.y - cube_height_above_floor,
                             bottom_left_corner.z + cube_size / 2.0f };

  // Floor first, cube second: cube is drawn over the floor.
  fill_floor_vectors_3d (world_vectors, floor_centre,
                         60.0f, // Width along X.
                         80.0f  // Depth along Z.
  );

  fill_cube_vectors_3d (world_vectors + floor_triangles * 3, bottom_left_corner, cube_size);

  camera main_camera = { world_origin, 0, 0, 0, 20, 40, FOV_input * (M_PI / 180.0f), HEIGHT / WIDTH };
  ...
  for (int row = 0; row < 2; row++)
    {
      for (int column = 0; column < 5; column++)
        {
          Uint32 colour = ((row + column) % 2 == 0) ? 0xFF707070u : 0xFFB0B0B0u;

          int first_triangle = (row * 5 + column) * 2;

          triangle_colours[first_triangle] = colour;
          triangle_colours[first_triangle + 1] = colour;
        }
    }
  ...
  const float ANGLE_SPEED = 1.0f * (M_PI / 180.0f); // degrees per frame (or rad, your choice
  ...
  while (running)
    {
      ...
      printf ("pos: (%f, %f, %f) | yaw: %f pitch: %f roll: %f\n", main_camera.position.x, main_camera.position.y,
              main_camera.position.z, main_camera.yaw, main_camera.pitch, main_camera.roll);
      ...
      rasterise_screen_coordinates (screen_vectors, triangle_number, triangle_colours, pixels, WIDTH, HEIGHT);
